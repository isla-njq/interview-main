<template>
  <div class="question-view">
    <div class="answer-go" @click="gotest">去做题</div>
    <div class="answer-show" @click="answerShow = !answerShow" v-text="answerTest"></div>
    <div class="question-name">
      <div class="title">123.&nbsp;&nbsp;&nbsp;{{questionsession.title}}</div>
      <div class="all-content">
        <p class="content" v-for="(item, index) in questionsession.content" :key="index">
          {{item}}
        </p>
      </div>
    </div>
    <div :class="[answerShow ? 'close question-answer' : 'open question-answer']">
      <h3>参考答案</h3>
      <div class="answer">
        <div>
          {{questionsession.content[0]}}
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    answerTest () {
      return this.answerShow ? '查看答案' : '收起答案'
    }
  },
  methods: {
    gotest () {
